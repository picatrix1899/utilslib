...
public class Point3f
{
	...
	public Point3f set(Vec3f v) { return set(v.x, v.y, v.z); }	
	...
	public Vec3f vectorTo(Point3f p) { return new Vec3f(p.x - this.x, p.y - this.y, p.z - this.z); }
	
	public Vec3f vectorFrom(Point3f p) { return new Vec3f(this.x - p.x, this.y - p.y, this.z - p.z); }
	
	public float distanceTo(Point3f p) { return vectorTo(p).length(); }
	
	public float distanceFrom(Point3f p) { return vectorFrom(p).length(); }
	
	public Vec3f directionTo(Point3f p) { return vectorTo(p).normalize(); }
	
	public Vec3f directionFrom(Point3f p) { return vectorFrom(p).normalize(); }
	
	public Vec3f vector() { return new Vec3f(this.x, this.y, this.z); }
	...
	public Point3f add(Vec3f v) { return add(v.x, v.y, v.z); }
	
	public Point3f add(float x, float y, float z) { return new Point3f(this.x + x, this.y + y, this.z + z); }
	
	public Point3f sub(Vec3f v) { return sub(v.x, v.y, v.z); }
